...
public class EmployeesWebPage extends WebPageWithPagination {
	...
	public void verifyTabsAreVisible() {
		new WebDriverWait(driver, 30)
		  .until(ExpectedConditions.visibilityOf(activetab));
	...
	public List<String> getActiveEmployees() {
		...
		List<WebElement> employeeact =  employeestable.getWrappedElement().findElements(By.xpath(".//tbody/tr[contains(@id, 'ctl00_ctl00_Content_Main_gv_ctl00')]/td[4]"));
	...
	public String verifyEmployeesDuplicatesArchived(List<String> employeeact) {
		...
		List<WebElement> usercolumnsarch =  archivedemployeestable.getWrappedElement().findElements(By.xpath(".//tbody/tr[contains(@id, 'ctl00_ctl00_Content_Main_gvDeleted_ctl00')]/td[3]"));
	...
	public boolean isActiveEmployeeExists(String firstname, String lastname) {
		...
		boolean exists =  employeestable.getWrappedElement().findElements(By.xpath(".//tr/td[text()='" + employeename + "']")).size() > 0;
	...
	public boolean isArchivedEmployeeExists(String firstname, String lastname) {
		String employeename = firstname + " " + lastname;
		new WebDriverWait(driver, 30)
		  .until(ExpectedConditions.visibilityOf(archivedemployeestable.getWrappedElement()));
		this.driver.manage().timeouts().implicitlyWait(3, TimeUnit.SECONDS);
		boolean exists =  archivedemployeestable.getWrappedElement().findElements(By.xpath(".//tr/td[text()='" + employeename + "']")).size() > 0;
		this.driver.manage().timeouts().implicitlyWait(30, TimeUnit.SECONDS);
		return exists;
	}
